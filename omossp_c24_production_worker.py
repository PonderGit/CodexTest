...
UA="OMOSSP-C24-ProductionWorker/1.0"
BATCH=Path(os.environ.get("OMOSSP_BATCH_FILE","production_batches/omossp_c18_batch3.json"))
OUT=Path("omossp_c24_output")
OUT.mkdir(exist_ok=True)

PACKAGE_CLASSES={"PACKAGE_CONTAINER","UNRESOLVED_SOURCE_CONTAINER"}
IDENTITY_BEARING_SPEC_FIELDS={"url","vcs"}
AUTO_ACCEPT={
...
def identity_eligible_origin(origin:dict)->bool:
    return (origin.get("origin")=="EC02_SPEC_FIELD" and str(origin.get("field","")).casefold() in IDENTITY_BEARING_SPEC_FIELDS)
...
def acquire(row):
    ...
    for root,d in githubs.items():
        ...
        if any(identity_eligible_origin(o) for o in origins) and state.get("meta_card") and readme_card:
    ...
    for (kind,ident),state in registry_state.items():
        if state["card"] and state["valid"] and any(identity_eligible_origin(o) for o in state["origins"]):
    ...
    for root,state in gitlab_state.items():
        if state["card"] and any(identity_eligible_origin(o) for o in state["origins"]):
            state["card"]["evidence_grade"]="P1"
...
if batch.get("protocol")!="OMOSSP_PHASE_C18_PRODUCTION_BATCH3_FREEZE_R1":
    raise SystemExit("C18 Batch-3 protocol mismatch")
if batch.get("authority_sha256")!="22d5205d1d89a01d45eb682e1ef672c621c5fd455d07f810ee8cbc04e9ad9bc8":
    raise SystemExit("C18 authority SHA mismatch")
if batch.get("membership_sha256")!="74c1b9ca2f362ea5cb755b50cadcf3a5820ab59891445bd79fc325da2f28ccdd":
    raise SystemExit("C18 Batch-3 stored membership SHA mismatch")
if batch.get("selection_freeze_sha256")!="8ca3a8bb179992c9fbb2ecc00fbba9be7ed2a3857bb4f8f072ba00b9c0f91ebc":
    raise SystemExit("C18 Batch-3 stored selection SHA mismatch")

rows_all=batch.get("rows") or []
if len(rows_all)!=100:
    raise SystemExit(f"C18 Batch-3 row-count mismatch: {len(rows_all)}")
if len({int(r["source_scale_rank"]) for r in rows_all})!=100:
    raise SystemExit("C18 duplicate source_scale_rank")
if any(any("performance" in k.lower() for k in r.keys()) for r in rows_all):
    raise SystemExit("C18 performance field leakage into production batch")
...
if sha(canon_rows)!=batch.get("membership_sha256"):
    raise SystemExit("C18 Batch-3 membership SHA mismatch")
canon_sel=json.dumps(batch["selection_rule"],ensure_ascii=False,sort_keys=True,separators=(",",":")).encode("utf-8")
if sha(canon_sel)!=batch.get("selection_freeze_sha256"):
    raise SystemExit("C18 Batch-3 selection SHA mismatch")
...
rows=rows_all[lo:hi]
if len(rows)!=25:
    raise SystemExit("C18 shard row-count mismatch")
...
OUT.mkdir(exist_ok=True)
(OUT/f"c24_batch3_shard{shard}.json").write_bytes(pretty)
(OUT/f"c24_batch3_shard{shard}.sha256").write_text(sha(canon)+"  canonical-json\n",encoding="utf-8")
print(json.dumps({
 "batch":"B003",
 "shard":shard,
 "rows":len(results),
