...
UA="OMOSSP-C13-ProductionWorker/1.0"
BATCH=Path(os.environ.get("OMOSSP_BATCH_FILE","production_batches/omossp_c13_batch2.json"))
OUT=Path("omossp_c13_output")
...
if batch.get("protocol")!="OMOSSP_PHASE_C13_PRODUCTION_BATCH2_FREEZE_R1":
    raise SystemExit("C13 Batch-2 protocol mismatch")
if batch.get("authority_sha256")!="22d5205d1d89a01d45eb682e1ef672c621c5fd455d07f810ee8cbc04e9ad9bc8":
    raise SystemExit("C13 authority SHA mismatch")
if batch.get("membership_sha256")!="1a801f5c6d9d98ebda950b3cf8d43676e091a7d335346495938d8f9548095b80":
    raise SystemExit("C13 Batch-2 stored membership SHA mismatch")
if batch.get("selection_freeze_sha256")!="7836029c70a7ce55e5efcf5af6fbb06ea7f7fd1038dedcb0957ebf9dd0a4442b":
    raise SystemExit("C13 Batch-2 stored selection SHA mismatch")

rows_all=batch.get("rows") or []
if len(rows_all)!=100:
    raise SystemExit(f"C13 Batch-2 row-count mismatch: {len(rows_all)}")
if len({int(r["source_scale_rank"]) for r in rows_all})!=100:
    raise SystemExit("C13 duplicate source_scale_rank")
if any(any("performance" in k.lower() for k in r.keys()) for r in rows_all):
    raise SystemExit("C13 performance field leakage into production batch")
...
if sha(canon_rows)!=batch.get("membership_sha256"):
    raise SystemExit("C13 Batch-2 membership SHA mismatch")
...
if sha(canon_sel)!=batch.get("selection_freeze_sha256"):
    raise SystemExit("C13 Batch-2 selection SHA mismatch")

shard=int(os.environ.get("OMOSSP_SHARD","1"))
if shard not in (1,2,3,4):
    raise SystemExit("C13 shard must be 1..4")
lo=(shard-1)*25
hi=lo+25
rows=rows_all[lo:hi]
if len(rows)!=25:
    raise SystemExit("C13 shard row-count mismatch")
...
payload={
 "protocol":"OMOSSP_PHASE_C13_PRODUCTION_SHARD_RESULT_R1",
...
OUT.mkdir(exist_ok=True)
(OUT/f"c13_batch2_shard{shard}.json").write_bytes(pretty)
(OUT/f"c13_batch2_shard{shard}.sha256").write_text(sha(canon)+"  canonical-json\n",encoding="utf-8")
print(json.dumps({
 "batch":"B002",
 "shard":shard,
 "rows":len(results),
