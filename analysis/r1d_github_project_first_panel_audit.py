...
SNAPSHOT=Path("governance/r1d_oss_compass_github_single_repository_snapshot_20260906.csv")
OUT_SUMMARY=Path("governance/r1d_github_project_first_panel_coverage_summary.json")
OUT_REPO=Path("governance/r1d_github_project_first_panel_coverage_by_repo.csv")

BASE="https://oss.open-digger.cn/github"
UA="OMOSSP-R1D-coverage-audit/1.0"
METRICS=[
    "issues_new",
    "issues_closed",
    "issue_response_time",
...
def load_candidates():
    ...
    return sorted(dedup.values(),key=str.casefold),len(exact)
...
def main():
    candidates,exact_n=load_candidates()
    rows=[]
    with ThreadPoolExecutor(max_workers=32) as ex:
        fut={ex.submit(inspect_repo,r):r for r in candidates}
        for i,f in enumerate(as_completed(fut),1):
            rows.append(f.result())
            if i%100==0: print(f"R1D_PROGRESS={i}/{len(candidates)}",flush=True)
    ...
    summary={
      "protocol":"OMOSSP_R1D_GITHUB_PROJECT_FIRST_PANEL_COVERAGE_R1",
      "candidate_source":"OSS Compass GitHub single-repositories snapshot",
      "candidate_rows_unique_exact_case":exact_n,
      ...
      "primary_duration_coverage_among_full24":{
        "issue_resolution_ge6_months":sum(int(r["issue_resolution_duration__n24"])>=6 for r in full24),
        "issue_resolution_ge12_months":sum(int(r["issue_resolution_duration__n24"])>=12 for r in full24),
        "pr_resolution_ge6_months":sum(int(r["change_request_resolution_duration__n24"])>=6 for r in full24),
        "pr_resolution_ge12_months":sum(int(r["change_request_resolution_duration__n24"])>=12 for r in full24),
        "either_resolution_family_ge6_months":sum(max(int(r["issue_resolution_duration__n24"]),int(r["change_request_resolution_duration__n24"]))>=6 for r in full24),
        "either_resolution_family_ge12_months":sum(max(int(r["issue_resolution_duration__n24"]),int(r["change_request_resolution_duration__n24"]))>=12 for r in full24),
        "both_resolution_families_ge6_months":sum(int(r["issue_resolution_duration__n24"])>=6 and int(r["change_request_resolution_duration__n24"])>=6 for r in full24),
        "both_resolution_families_ge12_months":sum(int(r["issue_resolution_duration__n24"])>=12 and int(r["change_request_resolution_duration__n24"])>=12 for r in full24)
      },
    ...
    print("===== OMOSSP_R1D_SUMMARY_BEGIN =====")
    print(json.dumps(summary,ensure_ascii=False,sort_keys=True))
    print("===== OMOSSP_R1D_SUMMARY_END =====")
